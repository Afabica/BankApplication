...
describe("SignInPage Component", () => {
  ...
  it("logs in successfully and redirects to dashboard", async () => {
    ...
    await waitFor(() => {
      ...
      expect(request.data).toContain("maria123"); // Check if data sent contains the username

      // ✅ Corrected expectation: match actual cookie setup
      expect(setCookie).toHaveBeenCalledWith(
        null, // context
        "jwt", // key
        "fake.jwt.token", // value
        expect.objectContaining({
          path: "/", // expected cookie path
          httpOnly: false, // fixed: match actual setting
          secure: true, // expected cookie security flag
          maxAge: 86400, // added if your app includes this
        }),
      );
